...
import { optionBDesktopMenus, optionBMobileSections, optionBFooterLinks } from '../../config/optionB';
          ...
          <nav className="flex items-center gap-1">
            {NAV_ITEMS.map(({ key, label }) => (
              ...
              >
                <button
                  type="button"
                  className={`rounded px-3 py-6 text-sm font-semibold transition-colors hover:text-black focus:outline-none focus-visible:ring-2 focus-visible:ring-[#333333] ${activeMenu === key ? 'text-black bg-[#E8E8E8]' : 'text-[#333333]'}`}
                  onClick={() => toast(`Would navigate to ${label}`)}
                ...
                <MegaMenu
                  ...
                  columns={optionBDesktopMenus[key].columns}
                  width={optionBDesktopMenus[key].width}
                />
              </div>
            ))}
          </nav>
  ...
  return (
    <>
      ...
      <MobileMenu
        ...
        sections={optionBMobileSections}
        footerLinks={optionBFooterLinks}
      />
    </>
  );
}
